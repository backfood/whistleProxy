...
function getLunarByBetween(year, month, day) {
  ...
  var between = getDaysBetweenSolar(
    year,
    zenMonth - 1,
    zenDay,
    year,
...
function lunarToSolar(_year, _month, _day) {
  ...
  var offDate =
    new Date(year, zenMonth - 1, zenDay).getTime() + between * 86400000;
...
function solarToLunar(_year, _month, _day) {
  ...
  //立春日期
  var term2 = cacheUtil.get('term2')
    ? cacheUtil.get('term2')
    : cacheUtil.set('term2', getTerm(year, 2));
  //二十四节气
  var termList = cacheUtil.get('termList')
    ? cacheUtil.get('termList')
    : cacheUtil.set('termList', getYearTerm(year));
  ...
  //除夕
  if (
    lunarDate[1] == lunarMonthDays.length - 1 &&
    lunarDate[2] == lunarMonthDays[lunarMonthDays.length - 1]
  ) {
...
function solarCalendar(_year, _month, fill) {
  ...
  if (fill) {
    if (res.firstDay > 0) {
      ...
      preMonthData = creatLenArr(
        preYear,
        preMonth + 1,
        res.firstDay,
        preMonthDays - res.firstDay + 1
      );
